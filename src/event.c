...
static void msg_redirect(char *buf, int len)
{
    ...
    int pipefd[2];
    if (pipe(pipefd) == -1) {
        srv_log(LOG_ERROR, "Failed to create pipe");
        return;
    }
    int n = write(pipefd[1], buf, len);
    if (n != len) {
        srv_log(LOG_ERROR, "write error to pipe");
        return;
    }
    splice(pipefd[0], NULL, STDIN_FILENO, NULL, len, SPLICE_F_MORE|SPLICE_F_MOVE);
    close(pipefd[0]);
    close(pipefd[1]);
#endif
}
